...
Page({
  ...
  onNavigatorTap: function (event) {
    ...
    var url = '../picker/picker?type=' + navigatorType + '&abc=1234567890'
    wx.navigateTo({
  ...
  logout: function () {
    ...
    wx.request({
      url: 'http://127.0.0.1:8000/api/v1.0/appes/logout/',
      method: 'GET',
      header: header,
      success: function (res) {
        console.log(res)
  ...
  getStatusFromRemote: function () {
    ...
    header.Cookie = cookie
    wx.request({
      url: 'http://127.0.0.1:8000/api/v1.0/appes/status/',
      method: 'GET',
      header: header,
      success: function (res) {
        console.log(res)
        if (res.data.is_authorized == 1) {
          ...
          console.log('0000session 过期,未登录状态')
        }
      }
    })
  },
